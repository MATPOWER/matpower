classdef sm_quad_cost_legacy < mp.set_manager_opt_model
...
%
% MP Set Manager class for legacy quadratic costs. The costs take one of two
...
%   See https://github.com/MATPOWER/mp-opt-model for more info.
    ...
    methods
        function obj = sm_quad_cost_legacy(varargin)
            % Constructor.
            % ::
            %
            %   qdc = mp.sm_quad_cost_legacy(label)
        ...
        function obj = add(obj, var, name, idx, varargin)
            ...
            if MQ
                if NQ ~= MQ && NQ ~= 1
                    error('mp.sm_quad_cost_legacy.add: Q (%d x %d) must be square or a column vector (or empty)', MQ, NQ);
                end
            end
            if Mc && Nc ~= 1
                error('mp.sm_quad_cost_legacy.add: c (%d x %d) must be a column vector (or empty)', Mc, Nc);
            end
            if MQ
                if Mc && Mc ~= MQ
                    error('mp.sm_quad_cost_legacy.add: dimensions of Q (%d x %d) and c (%d x %d) are not compatible', MQ, NQ, Mc, Nc);
                end
                nx = MQ;
            else
                if nv && ~Mc
                    error('mp.sm_quad_cost_legacy.add: Q and c cannot both be empty');
                end
                nx = Mc;
            end
            if nx ~= nv
                error('mp.sm_quad_cost_legacy.add: dimensions of Q (%d x %d) and c (%d x %d) do not match\nnumber of variables (%d)\n', MQ, NQ, Mc, Nc, nv);
            end
            ...
            if nargin > 2       %% individual set
                ...
                if isempty(idx)                 %% name, no index provided
                    if numel(obj.idx.i1.(name)) == 1    %% simple named set
                        ...
                        error('mp.sm_quad_cost_legacy.params: quadratic cost set ''%s'' requires an IDX_LIST arg', name);
                    end
        ...
        function obj = set_params(obj, var, name, idx, params, vals)
            ...
            if (N ~= N0 || nx ~= nx0) && ~is_all
                error('mp.sm_quad_cost_legacy.set_params: dimension change for ''%s'' not allowed except for ''all''', obj.nameidxstr(name, idx));
            end

            %% Q and c can't both be empty
            if ~MQ && ~Mc
                error('mp.sm_quad_cost_legacy.set_params: ''%s'' : ''Q'' and ''c'' cannot both be empty', obj.nameidxstr(name, idx));
            end

            %% check sizes of new values of Q, c, k
            if ~isempty(p.Q) && (MQ ~= nx || (MQ ~= NQ && NQ ~= 1) )
                error('mp.sm_quad_cost_legacy.set_params: ''%s'' : ''%s'' is expected to be (%d x %d)', obj.nameidxstr(name, idx), 'Q', MQ, NQ);
            end
            if ~isempty(p.c) && Mc ~= nx
                error('mp.sm_quad_cost_legacy.set_params: ''%s'' : ''%s'' is expected to be (%d x %d)', obj.nameidxstr(name, idx), 'c', Mc, 1);
            end
            if ~isempty(p.k) && any(p.k) && Nk ~= N && Nk ~= 1
                error('mp.sm_quad_cost_legacy.set_params: ''%s'' : ''%s'' is expected to be (%d x %d)', obj.nameidxstr(name, idx), 'k', N, 1);
            end
            ...
            if u.Q || u.c || u.vs
                ...
                if nx ~= nv
                    error('mp.sm_quad_cost_legacy.set_params: for ''%s'' dimensions of ''Q'', ''c'', ''k'' (%d) must be consistent with ''vs'' (%d)', obj.nameidxstr(name, idx), nx, nv);
                end
            end
            ...
            if obj.N
                ...
                elseif nargin < 5 || isempty(idx)   %% name, no idx provided
                    ...
                    elseif nargout == 1             %% indexing required, recurse
                        ...
                        error('mp.sm_quad_cost_legacy.eval: quadratic cost set ''%s'' requires an IDX_LIST arg when requesting DF output', name)
                    end
                else                                %% indexed named set
                ...
                for k = 1:nargout
                    switch tags{k}
                        ...
                        case 'd2f'
                            varargout{k} = d2f;
                        otherwise
                            error('mp.sm_quad_cost_legacy.get_soln: unknown tag ''%s''', tags{k});
                    end
                end
            end     %% if N
        end
