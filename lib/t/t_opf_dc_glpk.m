...
% t_opf_dc_glpk - Tests for legacy DC optimal power flow using GLPK solver.
...
if have_feature('glpk')
  for k = 1:length(algs)
    ...
    e(4) = 297.83776;
    e(7) = -26.94788;
    if k == 3, p = 2; else p = 4; end
    t_is(branch(:,MU_ANGMAX )-branch(:,MU_ANGMIN ), e, p, [t 'branch ang diff mu']);
