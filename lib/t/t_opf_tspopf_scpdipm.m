...
% t_opf_tspopf_scpdipm - Tests for legacy SCPDIPM-based optimal power flow.
...
t0 = 'SCPDIPMOPF : ';
mpopt = mpoption('opf.violation', 1e-6, 'pdipm.max_it', 100, 'pdipm.gradtol', 1e-7, ...
...
%     t_is(branch(:,ibr_mu    ), branch_soln(:,ibr_mu    ),  2, [t 'branch mu']);
    ...
    r = runopf(mpc, mpoption(mpopt, 'pdipm.comptol', 9e-8));
    [f, bus, gen, branch] = deal(r.f, r.bus, r.gen, r.branch);
    t_ok(r.success, [t 'success']);
    t_is(f, 11998.8345, 3, [t 'f']);
    t_is(gen(:, PG), [100.853092; 89.258007; 128.518423], 5, [t 'Pg']);
    t_is(gen(:, QG), [-20.834352; -7.966093; 34.591995], 4, [t 'Qg']);
    t_is([min(bus(:, VM)) mean(bus(:, VM)) max(bus(:, VM))], ...
...
%     t_is(r.mu.nln.u(61), 0, 6, [t 'issue #77']);
    ...
    t_is([min(r.bus(:, VM)) mean(r.bus(:, VM)) max(r.bus(:, VM))], ...
        [1.070692 1.090449 1.1], 3.9, [t 'bus voltage']);
else
