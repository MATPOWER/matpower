...
% t_opf_dc_osqp - Tests for legacy DC optimal power flow using OSQP solver.
...
mpopt = mpoption(mpopt, 'opf.dc.solver', 'OSQP');
osqp_opts = struct( ...
        'eps_rel', 5e-4, ...
        'rho', 0.01, ...
        'adaptive_rho_interval', 1000, ...
        'max_iter', 20000, ...
        'scaling', 1 ...
    );
osqp_opts2 = struct( ...
...
if have_feature('osqp')
    mpopt = mpoption(mpopt, 'osqp.opts', osqp_opts);
    ...
    mpopt2 = mpoption(mpopt, 'osqp.opts', osqp_opts2);
    r = rundcopf(mpc, mpopt2);
    ...
    t_skip(num_tests, 'OSQP not available');
end
